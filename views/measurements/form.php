			...
			<div class="col-md-12">
				<div class="panel_s">
					<div class="panel-body">
						<div class="_buttons">
							<a href="<?= admin_url('ella_contractors/measurements'); ?>" class="btn btn-default">Back</a>
						</div>
						<h4 class="no-margin mtop20"><?= html_escape($title); ?></h4>
						<hr class="hr-panel-heading" />

						<!-- Tab Navigation -->
						<ul class="nav nav-tabs mb-3" id="category-tabs">
							<li class="<?= ($category ?? 'siding') == 'siding' ? 'active' : ''; ?>">
								<a href="#siding-tab" data-toggle="tab" data-category="siding">Siding</a>
							</li>
							<li class="<?= ($category ?? 'siding') == 'roofing' ? 'active' : ''; ?>">
								<a href="#roofing-tab" data-toggle="tab" data-category="roofing">Roofing</a>
							</li>
							<li class="<?= ($category ?? 'siding') == 'windows' ? 'active' : ''; ?>">
								<a href="#windows-tab" data-toggle="tab" data-category="windows">Windows</a>
							</li>
							<li class="<?= ($category ?? 'siding') == 'doors' ? 'active' : ''; ?>">
								<a href="#doors-tab" data-toggle="tab" data-category="doors">Doors</a>
							</li>
						</ul>

						<form id="measurements-form" method="post" action="<?= admin_url('ella_contractors/measurements/save'); ?>">
							<input type="hidden" name="category" id="selected-category" value="<?= html_escape($category ?? 'siding'); ?>">
							
							<div class="tab-content">
								<!-- Siding Tab -->
								<div class="tab-pane <?= ($category ?? 'siding') == 'siding' ? 'active' : ''; ?>" id="siding-tab">
									<?php $this->load->view('ella_contractors/measurements/_form_modal', ['category' => 'siding', 'row' => ($row ?? null)]); ?>
								</div>
								
								<!-- Roofing Tab -->
								<div class="tab-pane <?= ($category ?? 'siding') == 'roofing' ? 'active' : ''; ?>" id="roofing-tab">
									<?php $this->load->view('ella_contractors/measurements/_form_modal', ['category' => 'roofing', 'row' => ($row ?? null)]); ?>
								</div>
								
								<!-- Windows Tab -->
								<div class="tab-pane <?= ($category ?? 'siding') == 'windows' ? 'active' : ''; ?>" id="windows-tab">
									<?php $this->load->view('ella_contractors/measurements/_form_modal', ['category' => 'windows', 'row' => ($row ?? null)]); ?>
								</div>
								
								<!-- Doors Tab -->
								<div class="tab-pane <?= ($category ?? 'siding') == 'doors' ? 'active' : ''; ?>" id="doors-tab">
									<?php $this->load->view('ella_contractors/measurements/_form_modal', ['category' => 'doors', 'row' => ($row ?? null)]); ?>
								</div>
							</div>
							
							<div class="text-right mtop15">
								<a href="<?= admin_url('ella_contractors/measurements'); ?>" class="btn btn-default">Cancel</a>
